...
def Fourier_descriptors (operator,Nb_coeff) :
    """
     Calculate the DFT of the operator's contour from a binarized image
     Return the first N coefficient and the number of disjoint contours found by the found_contours from skimage

     Input : A binrized operator image (40,40)
     Ouput : A tuple [First Nb_coeff of the DFT, number of disjoint contours]
    """
    # get the contours
...
def get_features(operators,Nb_coeff,training = False) :
    """
     Compute the features of a binarized operator image either for training or testing

     If training mode set to True :

        Input : (Nx3x40x40) arrays of operators loaded from load_train_data_knn with N number of rotations of the operator-> [addition,substraction,multiplication]
        Output : (3Nx4) array of features and (3N) array of targets -> [0,1,2]
                  Features : First four features are ratio of the amplitude of fourier descriptor -> [A1/A5,A2/A5,A3/A5,A4/A5]
                  Last features is the number of disjoint contour -> plus,minus,mul : 1, equal : 2, div :3

     If training mode set to False :

         Input : A binarized operator image (40,40)
         Output : The array of feature (1,5) [A1/A5,A2/A5,A3/A5,A4/A5,nb_contours]
    """
    if(training == True) :
